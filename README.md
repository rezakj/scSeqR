...
scSeqR is an R package that can analyze single sequencing data types (i.e [scRNA-Seq](https://en.wikipedia.org/wiki/Single_cell_sequencing) :+1:) and large matrix files. The program inputs single cell data in 10X format or standard matrix and data frames and helps you to perform QC, filtering, visualization, normalization, clustering, differential expression analysis and find positive and negative markers for each cluster.
